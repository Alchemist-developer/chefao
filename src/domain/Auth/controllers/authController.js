  ...
  async login(req, res) {
    try {
      ...
      if (!user) {
        return res
          .status(401)
          .json("Usuário ou Senha invalido, verique e tente novamente");
      }
      ...
      return res.json({ token, admin: user.admin, idUsers: user.idUsers, apartment: user.apartment, name: user.name });
    } catch (error) {
      console.error(error);
